...
void BST::insert( Node*& node, int d)
{
	if(node==nullptr)
	{
		...
		node->right=nullptr;			//putting into the right subtree
		node->left=nullptr;			//putting into the left subtree
		node->data=d;				//declare data of node equal to d
	}
...
{
		if(node==nullptr)			//if its empty
			return false;
		if(node->data== d)			//it found a match
			return true;
		else if(node->data<d)			//if a node value is less than previous node then search the right subtree of that node
			return search(node->right, d);
		else if(node->data>d)			//if node value is greater than previous node then search the left subtree of that node
...
****************************************************************/
int BST::sumOfRange( Node*& node, int lower, const int upper)
{
	int sum=0;
	...
	if(node->data <=lower && node->data>=upper)	//to see if its outside of the range
		return sumOfRange(node->right, lower, upper) + sumOfRange(node->left, lower, upper);
	else					//it should add all the nodes within the range
		return node->data+sumOfRange(node->left, lower, upper) + sumOfRange(node->right, lower, upper);
